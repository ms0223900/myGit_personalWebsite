...
<body>
	...
	<script>
			// 清空管理員帳密
 			document.getElementById("sigin-manager-Id").value = "" //管理員帳號
			document.getElementById("sigin-manager-Psw").value = "" //管理員密碼
			...
			xhr.onload = function (){
				if(xhr.status == 200){
					...
					if(xhr.responseText!=1 || xhr.responseText>100){ //此管理員帳密回傳的筆數
						// alert(xhr.responseText);
						alert("管理員帳密錯誤或權限不足");	
			...
			// alert("5" + document.getElementById("sigin-manager-Id").value);
			// alert("6" + document.getElementById("sigin-manager-Psw").value);								
			// alert(data_info);
			xhr.send(data_info);
