...
function initDayCook() {
    ...
    if($all('.grouponPayment-wrapper').length > 0) {
        ...
        for(let y in paymentMethod) {
            paymentMethod[y].onclick = function() {
...
function genChangeList(e) {
    ...
    if(this.id == 'checkChangBTN') { //
        ...
        $id('changeMealQRcode').src = 'http://chart.apis.google.com/chart?cht=qr&choe=UTF-8&chs=300x300&chl=http://140.115.236.72/demo-projects/CD103/CD103G3/' + changeMealURL;
...
function QRcodeAndCopyIt(thisGrouponNo) { //單一飯團用
    ...
    // QR
    $id('QR-picContainer').src = 'http://chart.apis.google.com/chart?cht=qr&choe=UTF-8&chs=300x300&chl=http://140.115.236.72/demo-projects/CD103/CD103G3/' + shareURL;
...
function achievementShowOff(getUrl) {
    ...
    function showAchie(jsonStr) {
        ...
        `<div class="grouponUser">
            ...
            <div class="userPic">
                <div class="pic">
                    <img src="images/member_pic/${founderAchie[1]}" alt="user">
                </div>
            </div>
