...
try {
    ...
    if($search == '' && $order == 'latest') {
        // if()
        $sql = "SELECT *,(memberNow / groupon_MemberNeed) as success from grouponorder by `groupon`.`groupon_No` DESC limit $page,6";
    } else if($order == 'latest') {
        $sql = "SELECT *,(memberNow / groupon_MemberNeed) as success from groupon where groupon_Name LIKE '$search' order by `groupon`.`groupon_No` DESC limit $page,6";
    } else if($order == 'endDate'){
        $sql = "SELECT *,(memberNow / groupon_MemberNeed) as success from groupon where groupon_Name LIKE '$search' order by `groupon`.`$order` ASC limit $page,6";
    } else if($order == 'success') {
        $sql = "SELECT * ,(memberNow / groupon_MemberNeed) as success from groupon where groupon_Name LIKE '$search' order by success DESC limit $page,6";
    } else if($order == 'official') {
        $sql = "SELECT * ,(memberNow / groupon_MemberNeed) as success from groupon where groupon_TagNo = 8 order by success DESC limit $page,6";
    }
...
if( $groupon -> rowCount() != 0) { //如果搜尋結果有配對成功
    ...
    foreach($grouponR as $i => $grouponRe) {
        ...
        if($grouponMeal -> rowCount() != 0) {
            ...
            $mealInGroupon = array(); //groupon最後塞進一個array
            ...
            foreach($grouponMealR as $j => $grouponMealR) {
                ...
                $thisMealList = array($mealName, $mealPic, $mealPrice,$mealScore, $mealKcal);
