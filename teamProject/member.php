...
<body onresize="myFunction()">
	...
	<div class="member">
		<div class="member-list">
			<div class="member-list-item">
				...
				<form method="POST" action="#" id="imgRULForm" target="nm_iframe">
					...
					<input style="display:none;" name="memberPic" id="memberPic" value="<?php echo $memRow->member_No ?>.png">
					<input style="display:none;" name="imgRUL" id="imgRUL" value="123">
				...
				<!-- <div id='ee' style="width:250px;height:250px;"></div> -->
				<!-- <img id='eee' src="" alt=""> -->
				<p class="member-list-item-contain">
					<span>  
						<img src="images/icon/user_white.svg" alt="member-Id" title="帳號" class="mem-icon">
						<?php echo $memRow->member_Nick ?>	
					</span>
					<span>
						<img src="images/icon/money_white.svg" alt="member-Bonus" title="購物金" class="mem-icon">
						<?php echo $memRow->member_Bonus ?>
					</span>
					<span>
						<img src="images/icon/mobile_white.svg" alt="mobile" title="手機" class="mem-icon">
			...
			<script>
				document.getElementById("subBTN-pic").addEventListener('click',function(){
					...
					html2canvas($("#member-Pic")[0]).then(function(canvas) {
						...
						$('#eee').attr("src",canvas.toDataURL("image/png"));
						$('#imgRUL').attr("value",canvas.toDataURL("image/png"));

						imgURL = canvas.toDataURL("image/png"); //取得截圖後的URL
					});

					//使用Ajax,更新登入者頭像
					setTimeout(() => {

						var formData = new FormData(document.forms["imgRULForm"]);
				...
				<ul class="member-order-list">	
					<?php 
						...
						try {
							...
							$sqlMemberOrder = "select * from memberOrder where 	member_No = " . $memRow->member_No;
					...
					<li>
						<?php
							if($prodRowMemberOrder->memOrder_status=='done'){
						?>
							<div class="ordering-end">
								已取餐
							</div>
						<?php
							}
	...
	<script>
		...
		for( i = 0 ; i < memberPanelLabel.length; i++){
			...
			// $(memberPanelLabel[i]).hover(
			// 	function() {
			// 		this.style.color = "#ff5000";
			// 	}, function() {
			// 		this.style.color = "#76391B";
			// 	}
			// );
		}
		...
		memberPanelLabel[0].style.color = "#ff5000"; //預設第一個
	</script>
